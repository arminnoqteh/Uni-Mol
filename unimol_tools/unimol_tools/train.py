...
class MolTrain(object):
    ...
    def fit(self, data):
        ...
        if "test_indices" in self.model.cv and len(self.model.cv["test_indices"]) > 0:
            ...
            if len(test_indices) > 0:
                ...
                self.model.load_best_model(best_model_path)  # Load the best model

                test_pred = self.model.predict(test_indices)
                ...
                # Save test results
                self.save_test_results(test_indices=test_indices)
